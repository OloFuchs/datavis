...
def plot_data(data_input: Path | pd.DataFrame, plot_params: list, plot_title:str = " ", y_label:str = " ",save_title: str = None, start_stop_time: tuple[int, int] | None = None, show_plot: bool = False, figsize: tuple[float, float] = (12, 8)) -> None:
    try:
        ...
        if isinstance(data_input, (str, Path)):
            ...
            raise ValueError("Invalid input: data_input must be either a path to a CSV file or a pandas DataFrame.")
        print(log.keys())
        ...
        if len(plot_params) == 1:
            fig, ax = plt.subplots(1, layout='constrained', figsize=figsize)  # Create a single subplot
            axs = [ax]  # Wrap it in a list to make it iterable in the loop
        else:
            fig, axs = plt.subplots(len(plot_params), 1, layout='constrained', figsize=figsize)
            ...
            for params in params_list:
                ...
                for idy, field in enumerate(fields):
                    ...
                    if field in log.columns:
                        ...
                        ax.plot(log["timestamps"], log[motor_field_name], label=legend, alpha = params.opacity)
                    else:
        ...
        if save_title:
            figures_dir = data_input.parent.parent / "figures"
            figures_dir.mkdir(parents=True, exist_ok=True)
            save_path = figures_dir / f"{save_title}.png"
...
def build_plot_param(params: PlotParameters) -> list:
    ...
    if params.field_name.upper() == "PENDANT":
        ...
        plot_params_list.append(params.field_name)
    else:
