...
def process_files(file_list, output_directory):
    try:
        ...
        conc = MDF.concatenate(
            [str(f) for f in file_list], direct_timestamp_continuation=True
        )
        # Extract bus logging
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process MF4 files from a directory or a zip file."
    )
    parser.add_argument(
        "--path",
        type=str,
        default="logs/05-13-24-warehouse-turning-test-2",
